...
\begin{tikzpicture}[scale=.76]
  \foreach \a/\label/\fillcolor in {90/F/reachgold,150/L/reachblue,210/L/reachblue,270/A/reachcopper,330/R/reachsteel,30/R/reachsteel} {
    \node[regular polygon,regular polygon sides=6,minimum size=1.0in,draw=white,fill=\fillcolor!32,rotate=30] at (\a:0.94in) {\sffamily\bfseries\label};
  }
  \node[regular polygon,regular polygon sides=6,minimum size=1.0in,draw=reachgold,line width=1.4pt,fill=reachvoid,rotate=30,text=white] at (0,0) {SHIP};
  \draw[->,reachgold,line width=1.2pt] (0,.2in)--(0,.66in);
  \node[align=center] at (0,-1.7in) {Adjacent arc reference; each arc continues outward.\\Boundary directions belong to both neighboring arcs.};
\end{tikzpicture}
\end{center}

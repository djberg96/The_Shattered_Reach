...
export function mountMatch(root) {
  ...
  const specialManeuverControls = (ship, timing) => ship.special_available ? `<section class="special-maneuvers"><header><div><b>Special maneuver token ready</b><small>Optional · use once per game · ${timing} normal movement</small></div></header><div class="special-maneuver-actions"><button class="secondary special" data-special="emergency_power">Emergency power <small>Extra forward move</small></button><button class="secondary special" data-special="quick_stop">Quick stop <small>Speed becomes zero</small></button><label>Bootlegger heading<select id="bootlegger-facing">${["East", "Northeast", "Northwest", "West", "Southwest", "Southeast"].map((label, direction) => `<option value="${direction}" ${ship.position[2] === direction ? "selected" : ""}>${label}</option>`).join("")}</select><button class="secondary bootlegger">Execute bootlegger <small>Rotate freely</small></button></label></div></section>` : "";
  const controls = (ship, target) => {
    ...
    if (state.phase === "allocation") {
      ...
      const specialManeuver = ship.size === "large" ? "" : `<fieldset class="special-allocation-status ${ship.special_available ? "ready" : "spent"}"><legend>Special maneuver token · no energy</legend><strong>${ship.special_available ? "Ready automatically" : "Expended"}</strong><p>${ship.special_available ? "Use once per game during movement: Bootlegger, Quick Stop, or Emergency Power." : "This ship has already used its one-time special maneuver."}</p></fieldset>`;
      ...
      return `<div class="control-stack"><label>Speed <output id="speed-value">${ship.allocation.speed}</output><input id="speed" type="range" min="0" max="12" value="${ship.allocation.speed}"></label><fieldset class="shield-allocation-list"><legend>Shield reinforcement</legend><label>Forward <output id="front-shields-value">${ship.allocation.shields.front}</output><input id="front-shields" type="range" min="0" max="${ship.shields.front > 0 ? shieldCap : 0}" value="${ship.allocation.shields.front}"></label><label>Aft <output id="aft-shields-value">${ship.allocation.shields.aft}</output><input id="aft-shields" type="range" min="0" max="${ship.shields.aft > 0 ? shieldCap : 0}" value="${ship.allocation.shields.aft}"></label></fieldset>${shieldRepair}${specialManeuver}<fieldset class="weapon-allocation-list"><legend>Weapon circuits</legend>${weaponChoices(ship)}</fieldset><div class="allocation-budget"><span>Energy committed</span><b><output id="energy-used">0</output> / ${ship.energy - ship.damage.engines}</b></div>${allocationActions}</div>`;
    }
    ...
    if (state.activity_step === "movement") {
      ...
      if (state.movement_stage === "after") return `<div class="control-stack">${activityStrip()}<p class="step-callout"><b>${movingShip.name} completed its normal movement.</b>Use its one-time special maneuver now, or preserve the token for a later impulse.</p>${specialManeuverControls(movingShip, "after")}<button class="phase-advance finish-movement">Preserve token and complete movement →</button>${undoMovement}</div>`;
  ...
  const bind = (ship, target) => {
    ...
    root.querySelectorAll(".move-ship, .movement-choice").forEach((button) => button.addEventListener("click", () => request("move_ship", { ship_id: state.pending_movement[0], maneuver: button.dataset.maneuver, offer_special_after: true })));
    root.querySelector(".finish-movement")?.addEventListener("click", () => request("finish_movement", { ship_id: state.pending_movement[0] }));

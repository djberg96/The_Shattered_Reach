...
export function mountMatch(root) {
  ...
  const render = () => {
    ...
    const commandTitle = state.phase === "allocation" ? "Commit your energy" : state.activity_step === "draw" && state.impulse === 11 ? "Draw the last impulse" : { draw: "Draw the next impulse", movement: "Choose your maneuver", launch: "Launch missiles", fire: "Resolve weapons fire" }[state.activity_step] || "Command the engagement";
    root.innerHTML = `

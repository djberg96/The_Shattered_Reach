...
export function mountMatch(root) {
  ...
  const controls = (ship, target) => {
    ...
    if (state.phase === "allocation") { const shieldCap = ship.size === "small" ? 1 : ship.size === "medium" ? 2 : 3; return `<div class="control-stack"><label>Speed <output id="speed-value">${ship.allocation.speed}</output><input id="speed" type="range" min="0" max="12" value="${ship.allocation.speed}"></label><fieldset class="shield-allocation-list"><legend>Shield reinforcement</legend><label>Forward <output id="front-shields-value">${ship.allocation.shields.front}</output><input id="front-shields" type="range" min="0" max="${ship.shields.front > 0 ? shieldCap : 0}" value="${ship.allocation.shields.front}"></label><label>Aft <output id="aft-shields-value">${ship.allocation.shields.aft}</output><input id="aft-shields" type="range" min="0" max="${ship.shields.aft > 0 ? shieldCap : 0}" value="${ship.allocation.shields.aft}"></label></fieldset><fieldset class="weapon-allocation-list"><legend>Weapon circuits</legend>${weaponChoices(ship)}</fieldset><div class="allocation-budget"><span>Energy committed</span><b><output id="energy-used">0</output> / ${ship.energy - ship.damage.engines}</b></div><button class="secondary save-allocation allocation-tip" aria-describedby="save-allocation-tip">Save draft<span id="save-allocation-tip" role="tooltip">Store this plan and keep it editable.</span></button><button class="primary commit-allocation allocation-tip" aria-describedby="commit-allocation-tip">Commit allocation<span id="commit-allocation-tip" role="tooltip">Save this plan and make it final for the turn${state.solo ? "; the AI then commits its own plan" : ""}.</span></button></div>`; }
  ...
  const render = () => {
    ...
    const selectedShip = state.ships.find((ship) => ship.id === selectedShipId && (damageReport || !state.solo || ship.player === player));
    ...
    root.innerHTML = `
      ...
      <main class="match-layout ${fleetStatusCollapsed ? "fleet-status-collapsed" : ""}"><section class="command-panel"><p class="eyebrow">${state.solo ? `Solo command · Your ship: ${current?.name}` : state.scenario === "tutorial" ? `Tutorial · ${["Set the battle plan", "Reveal allocations", "Choose a maneuver", "Fire your first weapon"][state.tutorial_step] || "Continue the engagement"}` : "Fleet command"}</p><h1 class="${state.phase === "allocation" ? "allocation-title" : ""}">${commandTitle}</h1>${commandLog}${current ? controls(current, target) : ""}</section>
